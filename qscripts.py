...
# Conjecture: Perhaps the numbers of the quintuple matter more for
# factorablity than the signs or order


def L1():  # Shows that not every quintuple can be factored using Quadrant 1 using Lipschitz#POSSIBLE ERROR
    for x in q_list:
        print len(quaternion.pyth_factor2(quaternion.Quaternion(0, 0, 0, 0, use_list=True, l=x), True)), x


def H1():  # Shows that every quintuple can be factored using Quadrant 1 using Hurwitz#POSSIBLE ERROR
    for x in q_list:
        print len(quaternion.pyth_factor(quaternion.Quaternion(0, 0, 0, 0, use_list=True, l=x), True, False)), quaternion.Quaternion(0, 0, 0, 0, use_list=True, l=x)


def L2():  # Shows the quintuples that are squares using Lipschitz#POSSIBLE ERROR
    for x in q_list:
        works = False
        factored_list = quaternion.pyth_factor2(
            quaternion.Quaternion(0, 0, 0, 0, use_list=True, l=x))
...
def H2():  # Shows the quintuples that are squares using Hurwitz#POSSIBLE ERROR
    for x in q_list:
        works = False
        factored_list = quaternion.pyth_factor(
            quaternion.Quaternion(
                0, 0, 0, 0, use_list=True, l=x))
        for y in factored_list:
            if y[0] == y[1]:
                works = True
        if works:
...
def L3():  # Prints the Lipschitz factorizations of integer quintuples in Quadrant 1
    # It appears that 1+2i+8j+10k and 2+4i+7j+10k cannot be factored, even
    # using Hurwitz quaternions.
    the_list = []
    for x in q_list:
        the_list.append(
            [
                quaternion.pyth_factor2(
                    quaternion.Quaternion(
                        0,
                        0,
                        0,
...
# print 3
    for y in the_list:
        ...
        for z in y[0]:  # y[0] is the list of factors, y[1] is the quintuple written as a quaternion
            print str(y[1]) + '=(' + str(z[0]) + ')*(' + str(z[1]) + ')'
# Every quintuple can be factored into two quaternions, each with the
# square root of the magnitude of the quintuple. However, it may be
# impossible for some quintuples to force its factors to use the same
# numbers.


def H4():  # Tries to find some Hurwitz factors of the quintuple 1,2,8,10 using the same number
    print[quaternion.pyth_factor(quaternion.Quaternion(
        1, 2, 8, 10), False, True), quaternion.Quaternion(1, 2, 8, 10)]
    print[quaternion.pyth_factor(
        quaternion.Quaternion(-1, 2, 8, 10), False, True), quaternion.Quaternion(-1, 2, 8, 10)]
    print[quaternion.pyth_factor(quaternion.Quaternion(
        1, -2, 8, 10), False, True), quaternion.Quaternion(1, -2, 8, 10)]
    print[quaternion.pyth_factor(quaternion.Quaternion(
        1, 2, -8, 10), False, True), quaternion.Quaternion(1, 2, -8, 10)]
...
# print 53
    for x in list:
        if x[0] % 2 and x[2] % 2 and x[2] % 2 and x[3] % 2:
            new.append([float(x[0]) / 2, float(x[1]) / 2,
                        float(x[2]) / 2, float(x[3]) / 2])
        else:
...
def X5():  # Tries every option to deal with 1,2,8,10,13
    goal = [1, 2, 8, 10]
    import os
    options = [[0, 0, 2, 3], [1, 2, 2, 2], [3.5, .5, .5, .5],
               [2.5, 2.5, .5, .5], [2.5, 1.5, 1.5, 1.5]]
    units1 = [quaternion.i(), quaternion.j(), quaternion.k(), 1]
    units2 = [quaternion.Quaternion(0.5, 0.5, 0.5, 0.5), quaternion.Quaternion(1.0, 0.0, 0.0, 0.0), quaternion.Quaternion(0.5, 0.5, 0.5, -
                                                                                                                          0.5), quaternion.Quaternion(0.5, 0.5, -
                                                                                                                                                      0.5, 0.5), quaternion.Quaternion(0.5, -
                                                                                                                                                                                       0.5, 0.5, 0.5), quaternion.Quaternion(0.5, 0.5, -
                                                                                                                                                                                                                             0.5, -
                                                                                                                                                                                                                             0.5), quaternion.Quaternion(0.5, -
                                                                                                                                                                                                                                                         0.5, -
                                                                                                                                                                                                                                                         0.5, 0.5), quaternion.Quaternion(0.5, -
                                                                                                                                                                                                                                                                                          0.5, 0.5, -
                                                                                                                                                                                                                                                                                          0.5), quaternion.Quaternion(0.5, -
            ...
            for s1 in signs:
                for s2 in signs:
                    for s3 in signs:
                        ...
                        for p in perms:
                            f1 = s0 * factor[0] * units1[p[0]] + s1 * factor[1] * units1[p[1]
                                                                                         ] + s2 * factor[2] * units1[p[2]] + s3 * factor[3] * units1[p[3]]

                            for S0 in signs:
                                for S1 in signs:
                                    for S2 in signs:
                                        for P in perms:
                                            f2 = S0 * factor[0] * units1[P[0]] + S1 * factor[1] * units1[P[1]
                                                                                                         ] + S2 * factor[2] * units1[P[2]] + factor[3] * units1[P[3]]
                                            for U in units2:
                                                product = f1 * f2 * U
