...
#    print len(results)
    ...
    f.write(
        "This program tried to count the number of 4D " +
        str(n) +
        "-tuples" +
        "\n")
    f.write(
        "This program searched over all factors whose greatest term in absolute value was " +
        str(m) +
        "\n")
    f.write("There are " + str(len(unique.keys())) +
            " equivalence classes that look different." + "\n")
