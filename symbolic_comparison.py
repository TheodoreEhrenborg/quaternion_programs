...
class Permutation:
    ...
    def __init__(self, starter=None):
        ...
        if set(self.__perm) != set(range(1, len(self) + 1)):
            raise Exception(
                "This Permutation was not input in the right format.")
    ...
    def apply(self, thing):
        ...
        if isinstance(thing, Permutation):
            ...
            raise Exception(
                "The Permutation could not be applied to the input.")

    def inverse(self):
        # The high numbers will cause an error if the next lines are wrong.
        temp = range(2 * len(self), 3 * len(self))
...
class Signed_Permutation(Permutation):
    ...
    def apply(self, thing):
        ...
        if isinstance(thing, Signed_Permutation):
            if len(thing) != len(self):
                raise Exception(
                    "The Signed_Permutations don't have the same length.")
            temp = []
            for x in range(1, len(self) + 1):
                temp.append(self.apply(thing.apply(x)))
            ...
            raise Exception(
                "The Signed_Permutation could not be applied to the input, since the input is of type " + str(type(thing)))
...
class Large_Transformation:
    ...
    units = [Quaternion_2(1, 0, 0, 0), Quaternion_2(-1, 0, 0, 0), Quaternion_2(0, 1, 0, 0), Quaternion_2(0, -1, 0, 0),
             Quaternion_2(0, 0, 1, 0), Quaternion_2(0, 0, -1, 0), Quaternion_2(0, 0, 0, 1), Quaternion_2(0, 0, 0, -1)]
    ...
    def __init__(self, quaternion_list, create_scalars=True):
        ...
        if not good:
            raise Exception(
                "This method got a list containing something that wasn't a Quaternion_2")
        new = []
    ...
    def apply(self, other):
        ...
        if len(perm_tuple) != len(self.__transformation_tuple):
            output = "The Large_Transformation has length " + \
                str(len(self.__transformation_tuple))
        ...
        for i in range(0, len(t) - 1):
            ...
            if first.get_conjugate() == second:
                for beta in Product.most_factors:
                    beta_bar = Product(
                        (beta,)).get_conjugate().get_perm_tuple()[0]
...
#                        print x.found
    count = 0
    file_name = "Results of looking for 4D " + \
        str(n) + "-tuples starting at " + str(int(start_time))
    f = open(file_name, "a")
    end_time = time.time()
...
def old_main3(n=2):
    ...
    while all_products_left != set():
        ...
        connected = connects_to_main(
            start, all_automorphisms, all_middle, all_end)
...
def main(n=2, time_between_reports=1800,
         reordering=False, alternate_list=None):
    #    '''Returns a tuple, where each element
    #    is a set of nodes that lie in the same
    #    connected graph'''
    import time
    ...
    short_transformations += Large_Transformation.get_all_middle_transformations(
        n)
    short_transformations += Large_Transformation.get_all_end_transformations(
        n)
    ...
    while len(connected) > 1:
        ...
        while time.time() - checkpoint < time_between_reports:
            ...
            temp = None
            while temp is None or not temp.is_in_most():
                if temp is None:
            ...
            if temp in current_eq_class:
                ...
                # Find the list in connected and add all of current_eq_class to
                # it.
...
#        all_products_left -= all_products_found_so_far
        ...
        f.write("\n\nHere's a human-readable version of the set of connected nodes.\n")
        f.write(
            "The program does not print any products that have a permutation that doesn't start with +1\n")
