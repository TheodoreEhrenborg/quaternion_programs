...
def main(n=5, how_many_factors = 2, file_name = None ):
    if how_many_factors == 2:
        ...
        raise Exception("There are no methods to handle " + str(how_many_factors) + " tuples")
...
#If the factors had norms of 5 or less
                                    ...
                                    coefficients = [abs(A), abs(B), 
                                                    abs(C), abs(D)]
                                    coefficients.sort()
                            #results[x-1] is the set of all products for this beta
                            ...
                            if a*a + b*b + c*c + d*d <= n*n:
                                for x in range(1,49):
                                    for y in range(1,49):                
                                        ...
                                        product = eval(alpha)*eval(current_beta)*eval(current_gamma)
#If the factors had norms of 5 or less
                                        A = product.a
                                        B = product.b
                                        C = product.c
                                        ...
                                        coefficients = [abs(A), abs(B), 
                                                        abs(C), abs(D)]
                                        coefficients.sort()
                            #results[48*(x-1)+y-1] is the set of all products for this beta
                                        results[48*(x-1)+y-1].add(tuple(coefficients))
                            #Note that coefficients is mutable, so it must become immutable
                            #before being added to the set
    total = set()
    unique = {}
    ...
    f.write("There are "+str(len(keys)) +" products that look different." + "\n")
    f.write("The products that look different have the following labels: "+ str(keys)  + "\n")
    for Y in range(0,len(keys)):
        for Z in range(Y+1,len(keys)):
            y = keys[Y]
            z = keys[Z]
            if True:
                f.write("Here are the values of Product "+ str(y) +" that Product " + str(z) + " does not have:" + "\n")
                f.write(str(unique[y] - unique[z]) + "\n")
                f.write("Here are the values of Product "+ str(z) +" that Product " + str(y) + " does not have:" + "\n")
...
#        print
    ...
    for x in keys:
        ...
        f.write(str(x) + " ("+alpha+")" + "("+current_beta+")" + "("+current_gamma+")"  + "\n")
                                ...
                                for x in range(1,49):
                                    for y in range(1,49):
                                        for z in range(1,49):
                                            current_beta = eval("beta" + str(x))
                                            current_gamma = eval("beta" + str(y))
                                            current_delta = eval("beta" + str(z))
                                            product = eval(alpha)*eval(current_beta)*eval(current_gamma)*eval(current_delta)
#If the factors had norms of 5 or less
                                            A = product.a
                                            B = product.b
                                            C = product.c
                                            ...
                                            coefficients = [abs(A), abs(B), 
                                                            abs(C), abs(D)]
                                            coefficients.sort()
                            #results[stuff] is the set of all products for this beta, gamma, delta
                                            results[48*48*(x-1)+48*(y-1)+(z-1)].add(tuple(coefficients))
                            #Note that coefficients is mutable, so it must become immutable
                            #before being added to the set
    total = set()
    unique = {}
...
#        print
    ...
    for x in keys:
        ...
        f.write(str(x) + " ("+alpha+")" + "("+current_beta+")" + "("+current_gamma+")" + "("+current_delta+")" + "\n")
    current_time = time.time()
    f.write("This program took "+ str(current_time - start) + " seconds" + "\n")
    f.write("This program considered " + str(len(total)) + " different solutions to the Diophantine equation." + "\n")
    f.close()
