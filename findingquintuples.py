        ...
        for b in range(a, end + 1):
            for c in range(b, end + 1):
                for d in range(c, end + 1):
                    ...
                    if int(e) == e and mymath.gcf(
                            mymath.gcf(mymath.gcf(a, b), c), d) == 1:
                        # print a,b,c,int(d)
                        l.append(
            ...
            for c in half_range(b, end + .5):
                for d in half_range(c, end + .5):
                    ...
                    if int(a + .5) == a + .5 and int(b + .5) == b + .5 and int(c + .5) == c + .5 and int(
                            d + .5) == d + .5 and mymath.gcf(mymath.gcf(mymath.gcf(a, b), c), d) == .5:
                        l.append(
                            [float(a), float(b), float(c), float(d), float(e)])
# and (int(a+.5)!=a+.5 or int(b+.5)!=b+.5 or int(c+.5)!=c+.5 or
# int(d+.5)!=d+.5):
    return l
