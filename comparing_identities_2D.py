...
class ComplexAdjoined:
    ...
    def __add__(self,other):
        ...
        return ComplexAdjoined( self.__a__ + other.get_a(), self.__b__ + other.get_b(), self.__m__)
    ...
    def __repr__(self):
        return str(self.__a__) + " + " + str(self.__b__) + " sqrt(" + str(self.__m__) + ")"
alpha = "complex(a,b)"
...
def old_main(n=5, how_many_factors = 2, file_name = None ):
    ...
    for x in range(0,num_possibilities):
        ...
        if results[x] not in unique.values():
            unique[x] = results[x]
    keys = unique.keys()
    f = open("Results of looking for 2D "+str(how_many_factors)+"-tuples " + file_name, "a") 
    f.write("\n\n")
...
def main(how_far = 5, how_many_factors = 2, file_name = None, radicand = -1, quiet = False ):
    ...
    for x in range(0, num_possibilities):
        for a in range(-how_far,how_far+1):
            for b in range(-how_far,how_far+1): #We could restrict b a little more based on the value of m
    ...
    f.write("\n\n")
    f.write("This program searched over all factors that had a norm of at most " + str(how_far*how_far)+"\n")
    f.write("There are "+str(len(keys)) +" products that look different." + "\n")
    f.write("The products that look different have the following labels: "+ str(keys)  + "\n")
    for Y in range(0,len(keys)):
        for Z in range(Y+1,len(keys)):
...
#                print y+1,
#        print
    f.write("The first product was composed of only these multiplied together: " + new_alpha + "\n")
